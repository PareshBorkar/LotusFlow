...
export default function LoginPage() {
  ...
  const [, setSession] = useLocalStorage<AuthSession | null>(STORAGE_SESSION_KEY, null);
  ...
  const displayError = apiError
    ? (apiError as any)?.data?.message || "We couldn't sign you in. Please try again."
    : authError;
    ...
    <main className='min-h-screen bg-[radial-gradient(circle_at_top,_#eff6ff,_#ffffff_45%,_#f8fafc_100%)] px-4 py-10 text-slate-950'>
      <div className='mx-auto flex min-h-[calc(100vh-5rem)] max-w-6xl items-center justify-center'>
        <section className='grid w-full overflow-hidden rounded-[32px] border border-slate-200 bg-white shadow-[0_30px_80px_rgba(15,23,42,0.12)] lg:grid-cols-[1.1fr_0.9fr]'>
          <div className='hidden bg-pink-300 px-10 py-12 text-white lg:flex lg:flex-col lg:justify-between'>
            <div className='space-y-6'>
              <div className='inline-flex rounded-full border border-white/15 bg-pink-500 px-4 py-1 text-sm tracking-[0.22em] uppercase'>
                LotusFlow
              </div>
              <div className='space-y-4'>
                ...
                <p className='max-w-lg text-base leading-7 text-white-300'>
                  Manage projects, teammates, and task updates from one shared workspace built for
            ...
            <div className='hidden grid gap-4 sm:grid-cols-3'>
              <div className='rounded-2xl border border-white/10 bg-white/5 p-4'>
                ...
                <p className='mt-2 text-sm text-white-300'>Tasks organized</p>
              </div>
              <div className='rounded-2xl border border-white/10 bg-white/5 p-4'>
                <p className='text-2xl font-semibold'>128</p>
                <p className='mt-2 text-sm text-white-300'>Teams onboarded</p>
              </div>
              <div className='rounded-2xl border border-white/10 bg-white/5 p-4'>
                <p className='text-2xl font-semibold'>99.9%</p>
                <p className='mt-2 text-sm text-white-300'>Uptime tracked</p>
              </div>
            </div>
          </div>
            ...
            <div className='mx-auto flex max-w-md flex-col justify-center'>
              <div className='mb-8 space-y-3'>
                ...
                <p className='text-sm leading-6 text-slate-600'>
                  Use any valid email and a password with at least 8 characters to continue through
                  the demo flow.
                </p>
              </div>
              ...
              <form className='space-y-5' onSubmit={handleSubmit} noValidate>
                ...
                <button
                  type='submit'
                  disabled={isLoading}
                  className='w-full rounded-2xl bg-pink-300 px-4 py-3 text-sm font-semibold text-white transition hover:bg-pink-500 disabled:cursor-not-allowed disabled:bg-slate-500'
                >
